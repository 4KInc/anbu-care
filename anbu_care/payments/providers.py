...
def create_order(*, payment_id: str, amount_inr: int, payee_label: str,
    ...
    The payee label rides along as a note so the order is legible in the
    provider's own dashboard. The destination itself is NOT sent: this is a
    collection into our account, and where money finally goes is the mandate's
    ...
    return ProviderResult(
        ...
        checkout_url=f"https://checkout.razorpay.com/v1/checkout.js#{body['id']}",
        detail=f"order {body['id']} created",
    )
